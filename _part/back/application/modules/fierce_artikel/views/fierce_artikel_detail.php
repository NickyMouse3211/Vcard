...
<div class="portlet light">
	...
	<div class="portlet-body">
		<div class="form-body">
			...
			<form method="POST" action="<?=base_url($instance.'/add_comment')?>" id='form-comment-artikel' class="form-horizontal form-comment" style="display:none;" role="form">
				<div class="form-body">
					<?php
					...
					<div class="form-group">
						...
						<div class="col-md-8">
							<textarea name="fiercecomment_isi" id='fiercecomment_isi' style='resize:none;' class="required form-control " placeholder="Add a comment . . " required></textarea>
							<span class="help-block"></span>
						</div>
					</div>
					<?php
						// echo "<pre>";
						// print_r ($this->session->userdata('user_data')->member_email);
						// echo "</pre>";
						if (@$this->session->userdata('user_data')->member_email == '')
						{
							...
							<div class="form-group">
								...
								<div class="col-md-8">
									<input type="email" class="required form-control" id='fiercecomment_email' name="email" placeholder="Your Email Address" required>
									<span class="help-block"></span>
								</div>
							</div>
							<?php
				...
				<div class="form-actions">
					<div class="row">
						<div class="col-md-offset-2 col-md-9">
							<input type="hidden" class="form-control" name="argaid" value="<?=$records->fierceartikel_id;?>" required>
							<button type="submit" class="btn btn-primary">Submit</button>
							<button type="button" class="btn btn-warning" onclick="$('#fiercecomment_isi').val('');">Clear</button>
						</div>
					</div>
				</div>
				<hr class="line_break">
...
<script type="text/javascript">
	var jenisarga   = '1';
	var idarga		= "<?=$records->fierceartikel_id?>";
			  ...
			  if (isConfirm) {
			  	$.post(url, data, function(res){
			  		var jumlahcoment=parseInt($('#jumlah-comment').html())+1;
			  	     ...
			  	     $('#fiercecomment_isi').val('');
			  	     $('#fiercecomment_email').val('');
