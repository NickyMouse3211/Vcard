          ...
          <div class='col-sm-11'>
            <div class='col-sm-12'>
              ...
              <div class='col-sm-2' align="right">
                <?php
                  if (@$key->member_nama != '' && @$this->session->userdata('user_data')->member_id == @$key->member_id || @$this->session->userdata('user_data')->member_role == '1') {
                    ?>
                    <i class="fa fa-pencil-square-o text-btn edit-button" coment='<?=$key->fiercecomment_id?>' data-toggle="tooltip" title="Edit Comment"></i>
                    <i class="fa fa-trash-o text-btn delete-button" coment='<?=$key->fiercecomment_id?>' data-toggle="tooltip" title="Delete Comment"></i>
                    <?php
                  }
                ?>
              </div>
            ...
            <div class='col-sm-12'>
              <div class='col-sm-9'>
                <textarea id='textarealabel<?=$i?>' class='textarealabel' desabled><?=$key->fiercecomment_isi?></textarea>
              </div>
              <div class='col-sm-3' align="right">
                on <?=$key->fiercecomment_insert_date?>
              </div>
            </div>
          </div>
...
<script type="text/javascript">
 $(document).ready(function() {
      ...
      $('.edit-button').click(function () {
        ...
        var url     = "<?=base_url('fierce_artikel/show_edit_comment')?>/";
        Custombox.open({
            target: url+'/'+id,
            effect: 'slip',
            width: '1000',
      ...
      $('.delete-button').click(function () {
        ...
        function(isConfirm){
          if (isConfirm) {
              var url     = "<?=base_url('fierce_artikel/delete_comment')?>/";
              var data    = [
                              {
                                name: "id_comment",
                                value: id
